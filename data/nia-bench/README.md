...
### Crash-Aware Reporting

Benchmark reporting is crash-aware:

- Runs that crash with no extracted code (`agentError != null` and `extractedFiles` empty) are excluded from quality analytics.
- `taskPassRate`, `hallucinationRate`, and `versionComplianceRate` are computed from non-crash runs only.
- Reports include both:
  - `meanCombinedScore` (includes crash runs as `0`)
  - `meanCombinedScoreCrashFree` (excludes crash-no-code runs)
- Reports also include explicit crash telemetry:
  - per-metric-group `crashRunCount` and `crashRate`
  - top-level `crashSummary` (`overall` + `byCondition`)
