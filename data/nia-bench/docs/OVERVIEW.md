...
generateAndWriteReport(runDir)
    ...
    │   ├─→ Crash-no-code runs are excluded from quality metrics
    │   │    (agentError != null and extractedFiles is empty)
    │   ├─→ taskPassRate: % of evaluated tasks with final_score >= 0.8
    │   ├─→ hallucinationRate: % of evaluated tasks with hallucinations
    │   ├─→ versionComplianceRate: % where ALL AST checks pass
    │   │    across evaluated runs (tasks without AST checks excluded)
    │   ├─→ meanCombinedScore: average final_score across all tasks
    │   │    (includes crash runs as 0)
    │   └─→ meanCombinedScoreCrashFree: average final_score across
    ...
    │   └─→ Excludes crash-no-code runs
    │
    ├─→ Extract per-task details
    │   └─→ Condition averages across evaluated reps
    │
    ├─→ Compute crashSummary
    │   └─→ overall + byCondition crash counts/rates
    │
    └─→ Write outputs:
...
| **Mean Combined Score**     | Weighted average incl. crash-as-zero   | Quality + reliability blend |
| **Mean Score (Crash-Free)** | Weighted average excluding crash-no-code | Pure quality signal       |
| **Crash Rate**              | % of runs with crash-no-code            | Runtime reliability         |
