...
describe("computeMetrics", () => {
	test("computes correct metrics for simple case", () => {
		...
		expect(metrics.meanCombinedScoreCrashFree).toBe(0.75);
		expect(metrics.evaluatedTaskCount).toBe(2);
		expect(metrics.evaluatedRunCount).toBe(2);
		expect(metrics.crashRunCount).toBe(0);
	...
	test("returns zeros for empty results", () => {
		...
		expect(metrics.meanCombinedScoreCrashFree).toBe(0);
		expect(metrics.evaluatedTaskCount).toBe(0);
		expect(metrics.evaluatedRunCount).toBe(0);
		expect(metrics.crashRunCount).toBe(0);
		expect(metrics.crashRate).toBe(0);
		expect(metrics.count).toBe(0);
	});

	test("excludes crash-no-code runs from passrate and quality analytics", () => {
		const results = [
			mockResult({
				taskId: "task-a",
				runIndex: 0,
			...
			mockResult({
				...
				hallucinations: {
					types: ["future_api"],
					details: [
						{ type: "future_api", evidence: "crash", description: "crash" },
					],
				},
				extractedFiles: {},
				agentError: {
		...
		expect(metrics.taskPassRate).toBe(1); // only non-crash rep counts
		expect(metrics.hallucinationRate).toBe(0); // crash hallucination noise excluded
		expect(metrics.meanCombinedScore).toBeCloseTo(0.475, 5);
		expect(metrics.meanCombinedScoreCrashFree).toBeCloseTo(0.95, 5);
		expect(metrics.evaluatedTaskCount).toBe(1);
		expect(metrics.evaluatedRunCount).toBe(1);
		expect(metrics.crashRunCount).toBe(1);
	...
	test("averages across reps for same task", () => {
		...
		expect(metrics.meanCombinedScoreCrashFree).toBeCloseTo(0.8, 5);
	...
	test("score formula: 2 pass + 1 fail", () => {
		...
		expect(metrics.meanCombinedScoreCrashFree).toBeCloseTo(
			(0.9 + 0.85 + 0.5) / 3,
			5,
		);
	});
...
describe("writeReport", () => {
	...
	test("JSON report is valid and parseable", async () => {
		...
		expect(parsed).toHaveProperty("crashSummary");
...
describe("formatReportText", () => {
	test("text report matches expected ASCII table format", async () => {
		...
		expect(text).toContain("Mean Score (Crash-Free)");
		expect(text).toContain("Crash Rate");
		expect(text).toContain("Crash Runs");
	...
	test("handles single condition without crashing", () => {
		const report = {
			...
			overall: [
				{
					...
					metrics: {
						...
						meanCombinedScoreCrashFree: 0.95,
			...
			taskDetails: [],
			crashSummary: {
				overall: { totalRuns: 1, crashRuns: 0, crashRate: 0 },
				byCondition: {
					nia: { totalRuns: 1, crashRuns: 0, crashRate: 0 },
				},
			},
		};
	...
	test("shows N/A for conditions with no data in a category", () => {
		const report = {
			...
			overall: [
				{
					...
					metrics: {
						...
						meanCombinedScoreCrashFree: 0.5,
						evaluatedTaskCount: 1,
						evaluatedRunCount: 1,
						crashRunCount: 0,
						crashRate: 0,
			...
			crashSummary: {
				overall: { totalRuns: 1, crashRuns: 0, crashRate: 0 },
				byCondition: {
					baseline: { totalRuns: 1, crashRuns: 0, crashRate: 0 },
					nia: { totalRuns: 0, crashRuns: 0, crashRate: 0 },
				},
			},
		};
	...
	test("empty report produces valid output", () => {
		const report = {
			...
			taskDetails: [],
			crashSummary: {
				overall: { totalRuns: 0, crashRuns: 0, crashRate: 0 },
				byCondition: {},
			},
		};
	...
	test("renders tool usage section when toolUsage data is present", () => {
		const report = {
			...
			overall: [
				...
				},
				{
					...
					metrics: {
						...
						meanCombinedScoreCrashFree: 0.8,
						evaluatedTaskCount: 1,
						evaluatedRunCount: 1,
						crashRunCount: 0,
						crashRate: 0,
				...
				},
				{
					...
					metrics: {
						...
						meanCombinedScoreCrashFree: 0.9,
						evaluatedTaskCount: 1,
						evaluatedRunCount: 1,
						crashRunCount: 0,
						crashRate: 0,
			...
			taskDetails: [],
			crashSummary: {
				overall: { totalRuns: 3, crashRuns: 0, crashRate: 0 },
				byCondition: {
					baseline: { totalRuns: 1, crashRuns: 0, crashRate: 0 },
					context7: { totalRuns: 1, crashRuns: 0, crashRate: 0 },
					nia: { totalRuns: 1, crashRuns: 0, crashRate: 0 },
				},
			},
		};
