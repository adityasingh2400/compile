...
export interface MetricsGroup {
	...
	/** Average final_score across all tasks (includes crash runs as 0) */
	meanCombinedScore: number;
	/** Average final_score across evaluated tasks only (excludes crash-no-code runs) */
	meanCombinedScoreCrashFree: number;
	/** Number of tasks with at least one non-crash run */
	evaluatedTaskCount: number;
	/** Number of non-crash runs in this group */
	evaluatedRunCount: number;
	/** Number of crash-no-code runs in this group */
	crashRunCount: number;
	/** % of runs in this group that crashed with no code */
	crashRate: number;
...
/**
 * Crash telemetry for benchmark reliability tracking.
 */
...
export interface TaskDetail {
	...
	conditions: Record<
		string,
		{
			...
			repCount: number;
			evaluatedRepCount: number;
			crashRepCount: number;
...
/**
 * A run is treated as a crash for analytics only when the agent errored and
 * produced no code files.
 */
function isCrashNoCodeRun(result: EvaluationResult): boolean {
	return (
		result.agentError != null && Object.keys(result.extractedFiles).length === 0
	);
}

/**
 * Computes crash telemetry for all runs and each condition.
 */
function computeCrashSummary(results: EvaluationResult[]): CrashSummary {
	const crashRuns = results.filter(isCrashNoCodeRun).length;
	const byConditionGroups = groupBy(results, (r) => r.condition);
	const byCondition: CrashSummary["byCondition"] = {};

	for (const [condition, condResults] of byConditionGroups) {
		const condCrashRuns = condResults.filter(isCrashNoCodeRun).length;
		byCondition[condition] = {
			totalRuns: condResults.length,
			crashRuns: condCrashRuns,
			crashRate:
				condResults.length > 0 ? condCrashRuns / condResults.length : 0,
		};
	}
	...
	return {
		overall: {
			...
			crashRuns,
			crashRate: results.length > 0 ? crashRuns / results.length : 0,
		},
		byCondition,
	};
}
...
export interface Report {
	...
	/** Explicit crash telemetry by condition and overall */
...
export function computeMetrics(results: EvaluationResult[]): MetricsGroup {
	const crashRunCount = results.filter(isCrashNoCodeRun).length;
	const evaluatedRunCount = results.length - crashRunCount;
	...
	// Group results by taskId to compute task-level metrics across non-crash reps.
	const taskGroups = groupByTask(results);
	const taskCount = taskGroups.size;

	let evaluatedTaskCount = 0;
	let passedTasks = 0;
	...
	for (const taskResults of taskGroups.values()) {
		// Average final score across all reps for this task (includes crash runs as 0).
		const avgFinalScoreAll =
			taskResults.reduce((sum, r) => sum + r.finalScore, 0) /
			taskResults.length;
		totalScoreAll += avgFinalScoreAll;

		const nonCrashResults = taskResults.filter((r) => !isCrashNoCodeRun(r));
		if (nonCrashResults.length === 0) {
			// Task had only crash-no-code reps; keep it out of quality analytics.
			continue;
		}

		evaluatedTaskCount++;

		// Average final score across evaluated reps only.
		const avgFinalScore =
			nonCrashResults.reduce((sum, r) => sum + r.finalScore, 0) /
			nonCrashResults.length;
		...
		const hasAnyAstCheck = nonCrashResults.some((r) => r.astResults.length > 0);
		if (hasAnyAstCheck) {
			tasksWithAstChecks++;
			const isCompliant = nonCrashResults.every((r) =>
	...
	return {
		taskPassRate: evaluatedTaskCount > 0 ? passedTasks / evaluatedTaskCount : 0,
		hallucinationRate:
			evaluatedTaskCount > 0 ? hallucinatedTasks / evaluatedTaskCount : 0,
		versionComplianceRate:
			tasksWithAstChecks > 0 ? compliantTasks / tasksWithAstChecks : 0,
		meanCombinedScore: taskCount > 0 ? totalScoreAll / taskCount : 0,
		meanCombinedScoreCrashFree:
			evaluatedTaskCount > 0 ? totalScore / evaluatedTaskCount : 0,
		evaluatedTaskCount,
		evaluatedRunCount,
		crashRunCount,
		crashRate: results.length > 0 ? crashRunCount / results.length : 0,
...
export function computeHallucinationDistribution(
	...
	for (const taskResults of taskGroups.values()) {
		const evaluatedResults = taskResults.filter((r) => !isCrashNoCodeRun(r));
...
export function buildTaskDetails(results: EvaluationResult[]): TaskDetail[] {
	...
	for (const [taskId, taskResults] of taskGroups) {
		...
		for (const [condition, condResults] of conditionGroups) {
			const evaluatedResults = condResults.filter((r) => !isCrashNoCodeRun(r));
			const crashRepCount = condResults.length - evaluatedResults.length;

			const avgFinalScore =
				evaluatedResults.length > 0
					? evaluatedResults.reduce((sum, r) => sum + r.finalScore, 0) /
						evaluatedResults.length
					: 0;
			const avgTestScore =
				evaluatedResults.length > 0
					? evaluatedResults.reduce((sum, r) => sum + r.testScore, 0) /
						evaluatedResults.length
					: 0;
			const avgJudgeScore =
				evaluatedResults.length > 0
					? evaluatedResults.reduce((sum, r) => sum + r.judgeScore, 0) /
						evaluatedResults.length
					: 0;

			// Collect all hallucination types across evaluated reps (deduplicated)
			...
			const totalToolCalls = evaluatedResults.reduce(
				(sum, r) => sum + (r.toolCallCount ?? 0),
				0,
			);
			const usedContextTool = evaluatedResults.some((r) =>
			...
			conditions[condition] = {
				...
				evaluatedRepCount: evaluatedResults.length,
				crashRepCount,
				totalToolCalls,
				usedContextTool,
			};
...
export async function generateReport(runDir: string): Promise<Report> {
	...
	// --- Crash telemetry ---
	const crashSummary = computeCrashSummary(results);
...
export function formatReportText(report: Report): string {
	...
	lines.push(separator);
	lines.push(
		" Note: Mean Combined Score includes crash runs as 0; Mean Score (Crash-Free) excludes crash-no-code runs.",
	);
	lines.push(separator);
	...
	for (const metricName of [
		...
		for (const cond of conditions) {
			...
			const evaluatedTaskCount = metrics?.evaluatedTaskCount ?? 0;
			...
			if (!metrics || count === 0) {
				...
				switch (metricName) {
					...
					case "Mean Combined Score":
						...
						break;
					case "Mean Score (Crash-Free)":
						value = formatMetricValue(
							metrics.meanCombinedScoreCrashFree,
							evaluatedTaskCount,
							false,
						);
						break;
					case "Crash Rate":
						value = formatMetricValue(metrics.crashRate, count, true);
						break;
					case "Crash Runs":
						value = `${metrics.crashRunCount}`;
						break;
				}
			}
	...
	if (report.taskDetails.length > 0) {
		...
		for (const detail of report.taskDetails) {
			...
			for (const cond of conditions) {
				...
				const value = condData
					? condData.evaluatedRepCount > 0
						? formatScore(condData.avgFinalScore)
						: "N/A"
					: "N/A";
				line += padLeft(value, colWidth);
			}
