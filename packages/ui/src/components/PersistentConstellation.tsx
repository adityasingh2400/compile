import { useEffect, useMemo, useRef } from "react";
import type { SyntheticCell } from "@compile/schemas";
import { HERO_CLUSTERS } from "../demo/fixtures.js";

/**
 * Globally-mounted constellation. Persists across phases 6 (stress_test),
 * 7 (clusters_revealed), and 8 (agent_writing) so the visual is one
 * unbroken animation rather than three disconnected canvases.
 *
 * Z-index choice: the canvas sits at z=1, page content at z=2. When the
 * active phase isn't a constellation phase, we fade the canvas out via
 * `visible` rather than unmounting (preserves point buffer state for the
 * inevitable replay).
 */

interface Props {
  cells: SyntheticCell[];
  visible: boolean;
  /** Page 7 — pulse cluster centroids and lighten background. */
  centroidsRevealed: boolean;
  /** Page 8 — dim everything to act as backdrop for the editor. */
  dimmed: boolean;
}

interface PointBuffer {
  x: Float32Array;
  y: Float32Array;
  tx: Float32Array;
  ty: Float32Array;
  r: Float32Array;
  g: Float32Array;
  b: Float32Array;
  ramp: Float32Array;
  count: number;
}

const MAX_POINTS = 12_000;

export function PersistentConstellation({
  cells,
  visible,
  centroidsRevealed,
  dimmed,
}: Props): JSX.Element {
  const canvasRef = useRef<HTMLCanvasElement>(null);
  const buf = useMemo<PointBuffer>(
    () => ({
      x: new Float32Array(MAX_POINTS),
      y: new Float32Array(MAX_POINTS),
      tx: new Float32Array(MAX_POINTS),
      ty: new Float32Array(MAX_POINTS),
      r: new Float32Array(MAX_POINTS),
      g: new Float32Array(MAX_POINTS),
      b: new Float32Array(MAX_POINTS),
      ramp: new Float32Array(MAX_POINTS),
      count: 0,
    }),
    [],
  );

  useEffect(() => {
    while (buf.count < cells.length && buf.count < MAX_POINTS) {
      const idx = buf.count;
      const cell = cells[idx]!;
      const cluster =
        HERO_CLUSTERS.find((c) => c.cluster_id === cell.cluster_id) ?? HERO_CLUSTERS[0]!;
      const edge = Math.floor(Math.random() * 4);
      buf.x[idx] = edge === 0 ? -1.05 : edge === 1 ? 1.05 : Math.random() * 2 - 1;
      buf.y[idx] = edge === 2 ? -1.05 : edge === 3 ? 1.05 : Math.random() * 2 - 1;
      const jitter = 0.07 + (1 - cluster.share) * 0.05;
      buf.tx[idx] = cluster.centroid[0] + (Math.random() - 0.5) * jitter * 2;
      buf.ty[idx] = cluster.centroid[1] + (Math.random() - 0.5) * jitter * 2;
      buf.r[idx] = cluster.color[0] / 255;
      buf.g[idx] = cluster.color[1] / 255;
      buf.b[idx] = cluster.color[2] / 255;
      buf.ramp[idx] = 0;
      buf.count++;
    }
  }, [cells, buf]);

  useEffect(() => {
    const canvas = canvasRef.current;
    if (!canvas) return;
    const ctx = canvas.getContext("2d");
    if (!ctx) return;
    const dpr = window.devicePixelRatio || 1;
    const resize = () => {
      canvas.width = canvas.clientWidth * dpr;
      canvas.height = canvas.clientHeight * dpr;
      ctx.setTransform(dpr, 0, 0, dpr, 0, 0);
    };
    resize();
    window.addEventListener("resize", resize);

    let raf = 0;
    let last = performance.now();
    const tick = (now: number) => {
      const dt = Math.min(0.05, (now - last) / 1000);
      last = now;
      const w = canvas.clientWidth;
      const h = canvas.clientHeight;
      ctx.fillStyle = "rgba(5, 6, 8, 0.18)";
      ctx.fillRect(0, 0, w, h);

      const cx = w / 2;
      const cy = h / 2;
      const scale = Math.min(w, h) * 0.42;
      const k = 2.6;
      for (let i = 0; i < buf.count; i++) {
        const dx = buf.tx[i]! - buf.x[i]!;
        const dy = buf.ty[i]! - buf.y[i]!;
        buf.x[i]! += dx * k * dt;
        buf.y[i]! += dy * k * dt;
        buf.x[i]! += (Math.random() - 0.5) * 0.0015;
        buf.y[i]! += (Math.random() - 0.5) * 0.0015;
        if (buf.ramp[i]! < 1) buf.ramp[i]! = Math.min(1, buf.ramp[i]! + dt * 0.4);
      }

      ctx.globalCompositeOperation = "lighter";
      const dimAlphaMul = dimmed ? 0.35 : 1;
      for (let i = 0; i < buf.count; i++) {
        const ramp = buf.ramp[i]!;
        const r = 0.9 * (1 - ramp) + buf.r[i]! * ramp;
        const g = 0.9 * (1 - ramp) + buf.g[i]! * ramp;
        const b = 0.95 * (1 - ramp) + buf.b[i]! * ramp;
        const alpha = (0.42 + ramp * 0.36) * dimAlphaMul;
        ctx.fillStyle = `rgba(${(r * 255) | 0}, ${(g * 255) | 0}, ${(b * 255) | 0}, ${alpha})`;
        const px = cx + buf.x[i]! * scale;
        const py = cy + buf.y[i]! * scale;
        ctx.fillRect(px, py, 1.5, 1.5);
      }
      ctx.globalCompositeOperation = "source-over";

      if (centroidsRevealed) {
        for (const c of HERO_CLUSTERS) {
          const px = cx + c.centroid[0] * scale;
          const py = cy + c.centroid[1] * scale;
          const radius = 14 + Math.sin(now * 0.003) * 3;
          const grd = ctx.createRadialGradient(px, py, 0, px, py, radius * 2);
          const a = dimmed ? 0.18 : 0.5;
          grd.addColorStop(0, `rgba(${c.color[0]}, ${c.color[1]}, ${c.color[2]}, ${a})`);
          grd.addColorStop(1, "rgba(0,0,0,0)");
          ctx.fillStyle = grd;
          ctx.beginPath();
          ctx.arc(px, py, radius * 2, 0, Math.PI * 2);
          ctx.fill();
        }
      }

      raf = requestAnimationFrame(tick);
    };
    raf = requestAnimationFrame(tick);
    return () => {
      cancelAnimationFrame(raf);
      window.removeEventListener("resize", resize);
    };
  }, [buf, centroidsRevealed, dimmed]);

  return (
    <canvas
      ref={canvasRef}
      style={{
        position: "fixed",
        inset: 0,
        width: "100%",
        height: "100%",
        background: "radial-gradient(circle at 50% 50%, #07090e 0%, #050608 70%, #030406 100%)",
        opacity: visible ? 1 : 0,
        transition: "opacity 600ms ease",
        pointerEvents: "none",
        zIndex: 1,
      }}
    />
  );
}
